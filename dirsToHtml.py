...
#
#
# Run with params: -c -I intro.txt -d .//ManolisMTzagarakis -T "Δημοσιέυσεις Εμμανουήλ Μ. Τζαγκαράκη"
#  
...
htmlCode = '<html><head> <meta http-equiv="Content-Type" content="text/html;charset=UTF-8"> <title>Ερευνητικό έργο Μανώλη Μ. Τζαγκαράκη</title><link rel="stylesheet" href="${CSSFILE}"></head><body id="topPage"><div id="container">\n<div id="cHeader">${TITLE}<br><div id="headerText">${INTROTEXT}</div></div> <div id="qnav"><div id="dirnavtitle">${DNAVTITLE}</div><div id="dirnav">${DIRNAV}</div></div> <div id="content"> <ul id="nestedlist">'
...
def folderOpenLink( linkF, displayF, henc=False, cl='#000000'):
    nVal = re.sub("\s\s+" , " ", linkF)
    if henc:
       nVal = nVal.replace(' ', '%20')
    
    return '<a href="' + nVal + '" target="_blank" rel="noopener noreferrer">' + '<font color="' + cl +  '"><b>'  + displayF + "</b></font></a>"
...
def scanDir(root=".//", lvl=1, vrb=False, henc=False, colorCycling=False, recDir = True, xList=[], iList=[], dirNav=None):
    ...
    for d in dirs:
        ...
        if colorCycling:
           ...
           dirNav.append( '<a href="#' + dId + '">' + d + "</a>")
           
        htmlCode = htmlCode + "<li id=\"" + dId + "\" class=\"dre child\"><font color=\"red\"><b>[</b></font>" +  folderOpenLink( os.path.join(root, d), d, False, clr) + '<font color=\"red\"><b>]</b></font>\n<ul>\n'
        if (recDir):
            nd, nf = scanDir( os.path.join(root, d), lvl+1, vrb, henc, colorCycling, recDir, xList, iList, dirNav)
            nDirs += nd
            nFiles += nf
        htmlCode = htmlCode + "</ul></li>\n"
        nDirs +=1
...
nD, nF = scanDir( rootDir, 1, verbose, htmlEncode, cCycling, recursiveD, excludedList, includedList, dirNavList)
...
htmlCode = htmlCode.replace("${DIRNAV}", dN)
if (len(dirNavList) == 0 ):
    dNTitle = ''
else:
    dNTitle = 'Quick Navigation Section'
    
htmlCode = htmlCode.replace("${DNAVTITLE}", dNTitle)
